...
currentVersionString=`grep -E 's.version.*=' JJSwiftLog.podspec`
...
lineNumber=`grep -nE 's.version.*=' JJSwiftLog.podspec | cut -d : -f1`

sed -i "" "${lineNumber}s/${currentVersion}/${newVersion}/g" JJSwiftLog.podspec
...
pod trunk push ./JJSwiftLog.podspec --verbose --allow-warnings
